...
public class ProgramRepositoryWithBagRelationshipsImpl implements ProgramRepositoryWithBagRelationships {
    ...
    public Optional<Program> fetchBagRelationships(Optional<Program> program) {
        return program.map(this::fetchSubjects);
    }
    ...
    public List<Program> fetchBagRelationships(List<Program> programs) {
        return Optional.of(programs).map(this::fetchSubjects).orElse(Collections.emptyList());
    }

    Program fetchSubjects(Program result) {
        return entityManager
            .createQuery("select program from Program program left join fetch program.subjects where program.id = :id", Program.class)
            .setParameter(ID_PARAMETER, result.getId())
            .getSingleResult();
    }

    List<Program> fetchSubjects(List<Program> programs) {
        ...
        List<Program> result = entityManager
            .createQuery("select program from Program program left join fetch program.subjects where program in :programs", Program.class)
